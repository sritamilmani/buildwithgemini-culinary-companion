...
_A2UI_TAG_RE = re.compile(r"</?(?:a2a_datapart_json|a2ui-json)>")
_A2UI_KEYS = ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface", "components", "surfaceId")


def _try_parse_a2ui_text(text: str) -> list[dict] | None:
    if "<a2ui-json>" in text or "components" in text:
        clean_text = _A2UI_TAG_RE.sub("", text).strip()
        ...
        try:
            ...
            if idx != -1:
                ...
                if isinstance(val, dict) and any(k in val for k in _A2UI_KEYS):
                    if "components" in val and "surfaceUpdate" not in val:
                        return [{"surfaceUpdate": val}]
                    return [val]
        except Exception:
            pass
    return None
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            txt = root.text
            parsed_a2ui = _try_parse_a2ui_text(txt)
            if parsed_a2ui:
                for a2m in parsed_a2ui:
                    out.append({"kind": "a2ui", "data": a2m})
            else:
