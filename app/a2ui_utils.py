...
def _normalize_message(msg: dict) -> dict:
    if "components" in msg and "surfaceUpdate" not in msg:
        return {"surfaceUpdate": msg}
    return msg
...
def _extract_a2ui_messages(text: str) -> list[dict]:
    ...
    for value in values:
        ...
        if isinstance(inner, dict) and any(k in inner for k in _A2UI_KEYS):
            messages.append(_normalize_message(inner))
        elif any(k in value for k in _A2UI_KEYS):
            messages.append(_normalize_message(value))
    return messages
