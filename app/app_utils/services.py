...
def get_memory_service():
    """Process-wide memory service: Vertex AI Memory Bank when ID is set, else in-memory."""
    ...
    engine_id = os.environ.get("MEMORY_BANK_ID") or os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_ID")
    if engine_id:
        from google.adk.memory import VertexAiMemoryBankService

        return VertexAiMemoryBankService(
            project=os.environ.get("GOOGLE_CLOUD_PROJECT") or "qwiklabs-gcp-03-f55cf09067a8",
            location=os.environ.get("GOOGLE_CLOUD_AGENT_ENGINE_LOCATION")
            or os.environ.get("GOOGLE_CLOUD_LOCATION")
            or "us-east1",
